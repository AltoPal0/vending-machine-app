...
import { useWeb3 } from './Web3Context';  // Manages Web3, account and connect Wallet
...
export default function Home() {
	const { web3, account, connectWallet } = useWeb3(); // New: Access context-provided values
	...
	// Initialize contracts and fetch ETH balance when web3 and account are set
	useEffect(() => {
		if (web3 && account) {
			const fetchBalancesAndContracts = async () => {
				try {
					// Fetch ETH balance
					...
					// Initialize token and sale contracts
					const tokenContractInstance = new web3.eth.Contract(ERC20_ABI, ERC20_CONTRACT_ADDRESS);
					setTokenContract(tokenContractInstance);

					const saleContractInstance = new web3.eth.Contract(TOKEN_SALE_ABI, TOKEN_SALE_ADDRESS);
					...
					setTokenPrice(parseFloat(priceInEth));
				} catch (error) {
					console.error('Error fetching balances or initializing contracts:', error);
				}
			};
			fetchBalancesAndContracts();
		}
	}, [web3, account]); // Re-run if web3 or account changes

	const handleBuyTokens = async () => {
		if (!tokenPrice || !tokenContract || !account || !numTokens || isNaN(Number(numTokens)) || Number(numTokens) <= 0) {
		  alert('Please ensure wallet is connected, token price is initialized, and enter a valid token quantity.');
		  return;
		}
		...
		try {
		  ...
		  const purchaseTx = await saleContract.methods.buyTokens(numTokens).send({
			from: account,
		  ...
		  setNumTokens(''); // Clear input
		  setBuyingTokens(false); // Collapse section
		} catch (error) {
